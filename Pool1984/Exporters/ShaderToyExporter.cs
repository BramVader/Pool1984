...
namespace Ajv.Pool1984.Exporters
{
    class ShaderToyExporter : Exporter
    {
        ...
        {
            string getNewLine(int indnt) => "\r\n" + new string(' ', indnt * 4);
            bool splitLines(string str) => str.Contains('\r') || str.Length > 60;
        ...
        public override void Export(Stream outputStream, Model model)
        {
            ...
            sb.AppendLine();
            sb.AppendLine("const vec3[] LIGHT_POS = vec3[] (" + String.Join(", ", model.Lights.OfType<SpotLight>().Select(lt => VecToString(lt.Center))) + ");");
            sb.AppendLine("const float[] LIGHT_RAD1 = float[] (" + String.Join(", ", model.Lights.OfType<SpotLight>().Select(lt => DoubleToString(lt.Radius1))) + ");");
            sb.AppendLine("const float[] LIGHT_RAD2 = float[] (" + String.Join(", ", model.Lights.OfType<SpotLight>().Select(lt => DoubleToString(lt.Radius2))) + ");");
            sb.AppendLine("const vec3[] COLOR = vec3[] (" + String.Join(", ", model.Primitives.OfType<Ball>().Select(ball => ColToString(ball.DiffuseColor))) + ");");
            sb.AppendLine();
